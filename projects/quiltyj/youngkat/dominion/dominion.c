...
int Baron(int choice1, struct gameState *state, int currentPlayer)
{
	...
	if (choice1 > 0) {//Boolean true or going to discard an estate
		...
		while (card_not_discarded) {
			if (state->hand[currentPlayer][p] == estate) {//Found an estate card!
			...
			else if (p > state->handCount[currentPlayer + 1]) {
				if (DEBUG) {
				...
				if (supplyCount(estate, state) > 0) {
					...
					state->supplyCount[estate] = 0;//Set supplyCount to zero
					if (supplyCount(estate, state) == 0) {
						isGameOver(state);
					}
				}
...
int Tribute(struct gameState *state, int handPos, int currentPlayer, int nextPlayer)
{
	...
	if ((state->discardCount[nextPlayer] + state->deckCount[nextPlayer]) <= 1) {
		...
		if (state->deckCount[nextPlayer] == 0) {
			...
			shuffle(nextPlayer + 1, state);//Shuffle the deck
		}
		tributeRevealedCards[0] = state->deck[nextPlayer][state->deckCount[nextPlayer] - 1];
...
int Mine(int choice1, int choice2, struct gameState *state, int handPos, int currentPlayer)
{
	...
	gainCard(choice2, state, 2.5, currentPlayer);
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  //int tributeRevealedCards[2] = {-1, -1}; Note: Moved to Tribute() function
  ...
  switch( card ) 
    ...
    case mine:
		//Call Mine function
		return Mine(choice1, choice2, state, handPos, currentPlayer);
    ...
    case minion:
		...
		return Minion(choice1, choice2, state, handPos, currentPlayer);
    ...
    case tribute:
		...
		return Tribute(state, handPos, currentPlayer, nextPlayer);
      
    case ambassador:
		//Call Ambassador function
		return Ambassador(choice1, choice2, state, handPos, currentPlayer);
