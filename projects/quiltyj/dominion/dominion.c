...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case mine:
    	selectMine(state, currentPlayer, choice1, choice2, handPos);
    	return 0;
    ...
    case baron:
      selectBaron(choice1, choice2,  choice3, state, handPos,bonus, currentPlayer);
      return 0;
    ...
    case minion:
    	selectMinion(choice1, choice2, choice3, state, handPos, bonus, currentPlayer);
    	return 0;
    ...
    case ambassador:
     selectAmbassador(choice1, choice2, state, currentPlayer, handPos);
     return 0;
...
void selectBaron(int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus, int currentPlayer)
{
	...
	if (choice1 > 0)
	{
		...
		while (card_not_discarded)
		{
			...
			{
				state->coins +=4; //adding four coins
				...
				state->hand[currentPlayer][state->handCount[currentPlayer]] = 1; //Intentional bug, should be -1
				state->handCount[currentPlayer]--;

				card_not_discarded = 0; //loop exit
			}

			else if (p  < state->handCount[currentPlayer]) //Intentional bug, should be p > state
			{
				if (DEBUG)
				{
				...
				if (supplyCount(estate, state) > 0)
				{
					...
					state->supplyCount[estate]--; //decrement estate
...
void selectMinion(int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus, int currentPlayer)
{
      ...
      if (choice1)		//+2 coins
	  {
    	  state->coins = state->coins - 2; //Intentional bug, should be + 2
	  }
	  ...
	  for (int i = 0; i < state->numPlayers; i++)
	  {
	  	if (i != currentPlayer)
	  	{
	  		if ( state->handCount[i] > 4 )
		    {
		      ...
		      for (int j = 0; j < 4; j++)
			  {
		    	  drawCard(j, state); //Intentional change, should be i
			  }
...
int selectAmbassador(int choice1, int choice2, struct gameState *state, int currentPlayer, int handPos)
{
	...
	if (j < choice2)
	{
		return 1; //Intentional bug, should be -1
	}
    ...
    for (j = 0; j < choice1; j++) //intentional bug, should be choice2
	{
		for (int i = 0; i < state->handCount[currentPlayer]; i++)
	    {
...
void selectTribute(struct gameState *state, int currentPlayer, int nextPlayer, int* tributeRevealedCards)
{
	if ((state->discardCount[nextPlayer] + state->deckCount[nextPlayer]) <= 1)
	{
		...
		else if (state->discardCount[nextPlayer] >= 0) //intentional bug, should be greater than
		{
			tributeRevealedCards[0] = state->discard[nextPlayer][state->discardCount[nextPlayer]-1];
	  		state->discardCount[nextPlayer]--;
		}
	...
	else
	{
		if (state->deckCount[nextPlayer] == 0)
		{
	  		for (int i = 0; i < state->discardCount[nextPlayer]; i++)
			{
	    		...
	    		state->deckCount[nextPlayer]++;
	    		state->discard[nextPlayer][i] = -1;
	    		state->discardCount[nextPlayer]++; //Intentional, should be --
	  		}
      ...
      if (tributeRevealedCards[0] == tributeRevealedCards[1]) //If we have a duplicate card, just drop one
	  {
		state->playedCards[state->playedCardCount] = tributeRevealedCards[1];
		state->playedCardCount++;
		tributeRevealedCards[1] = 1; //Intentional bug, should be -1
      }
...
{
	struct gameState* j;
	j->hand[currentPlayer][choice1] = state->hand[currentPlayer][choice1];  //store card we will trash
