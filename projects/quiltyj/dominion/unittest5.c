...
int main () 
{
	...
	printf ("Starting the test for Mine Card.\n");
	
	initializeGame(2, k[10], seed, TheGame);
	
	selectMine(TheGame, currentPlayer, choice1, choice2, handPos);

	
	//checking that it accurately increments the deck count, can access via a variable
	if (TheGame->hand[currentPlayer][choice1] == gardens)
		printf("Mine is not working!\n");
	else
		printf("Mine works!\n");
